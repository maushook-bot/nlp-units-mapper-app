...
class CreateConnection:
    def __init__(self, domain, local_db_name):
        ...
        self.uuid = pd.read_sql(f"SELECT c.tia_uuid FROM customer c WHERE c.domain = '{domain}';", con=app_core)
        self.prod_db = pd.read_sql(f"SELECT c.db_name_prod FROM customer c WHERE c.domain = '{domain}';", con=app_core)
    ...
    def local_engine_connection(self):
        ...
        local = create_engine(f"mysql+pymysql://{self.LOCAL_USER}:{self.LOCAL_PASSWORD}@{self.LOCAL_HOST}/{self.local_db_name}")
    ...
    def sandbox_engine_connection(self):
        ...
        sandbox = create_engine(f"mysql+pymysql://{self.MYSQL_USER_TRACK_STAGE}:{self.PASSWORD_SANDBOX}@{self.HOST_SANDBOX}/kart")
    ...
    def prod_engine_connection(self):
        ...
        prod = create_engine(f"mysql+pymysql://{self.MYSQL_USER_TRACK_PROD}:{self.PASSWORD_PROD}@{self.HOST_PROD}/{self.prod_db}")
